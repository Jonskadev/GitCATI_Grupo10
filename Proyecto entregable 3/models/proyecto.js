...
proyectos.updateProyecto = function(proyectoData, callback) {
    if (connection) {
        var sql = 'UPDATE Proyecto SET nombre = ' + connection.escape(proyectoData.nombre) + ', link = ' + connection.escape(proyectoData.link) + ' WHERE idProyecto = ' + connection.escape(proyectoData.idProyecto);
...
//cambiar valor activo
proyectos.updateActivo = function(proyectoData, callback) {
    if (connection) {
        var sql = 'UPDATE Proyecto SET activo = ' + connection.escape(proyectoData.activo) + ' WHERE idProyecto = ' + connection.escape(proyectoData.idProyecto);
        connection.query(sql, function(error, result) {
            if (error) {
                throw error;
            } else {
